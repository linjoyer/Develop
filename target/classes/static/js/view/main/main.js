...
layui.config({
    ...
    //图片总数
    $.get("/view/json/updatelog.json",
        function (data) {
            var strs="";
            for (var i = 0; i < data.length; i++) {
               strs+="<p>"+data[i].msg+"</p>";
            }
            $("#update_log").html(strs);
        }
    )
    //图片总数
    $.get("/view/json/images.json",
        function (data) {
